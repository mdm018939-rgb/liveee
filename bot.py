...
# 🔧 Configuration
# ══════════════════════════════════════════════════════════
BOT_TOKEN_1 = os.environ["BOT_TOKEN_1"]
...
bot1 = telebot.TeleBot(BOT_TOKEN_1, parse_mode="HTML")
bot2 = telebot.TeleBot(BOT_TOKEN_2, parse_mode="HTML")
...
# ══════════════════════════════════════════════════════════
# 🗑️ Auto-Delete Scheduler — একটা মাত্র ব্যাকগ্রাউন্ড থ্রেড দিয়ে
...
def schedule_delete(sender_bot, chat_id, message_id):
    """একটা মেসেজকে ৩ মিনিট পর ডিলিট হওয়ার জন্য কিউতে যোগ করে"""
    delete_at = time.time() + AUTO_DELETE_SECONDS
    with delete_queue_lock:
        delete_queue.append((delete_at, sender_bot, chat_id, message_id))

def delete_scheduler_worker():
    """প্রতি ৫ সেকেন্দে কিউ চেক করে, যাদের সময় হয়ে গেছে তাদেরই ডিলিট করে"""
    while True:
        now = time.time()
        due = []
        with delete_queue_lock:
            ...
            for item in delete_queue:
                delete_at, sender_bot, chat_id, message_id = item
                if delete_at <= now:
                    due.append(item)
                else:
                    remaining.append(item)
            delete_queue[:] = remaining

        for delete_at, sender_bot, chat_id, message_id in due:
            try:
                sender_bot.delete_message(chat_id, message_id)
                print(f"🗑️ মেসেজ {message_id} ডিলিট হয়েছে")
            except Exception as e:
                print(f"⚠️ মেসেজ ডিলিট করতে ব্যর্থ ({message_id}): {e}")
                ...
                if json_data.get("meta", {}).get("code") == 200:
                    ...
                    for service_index, service in enumerate(services):
                        try:
                            ...
                            # 🎯 আপনার কাঙ্ক্ষিত ফরম্যাট অনুযায়ী মেসেজ তৈরি
                            ...
                            color1 = colors[service_index % 3]
                            color2 = colors[(service_index + 1) % 3]
                            markup = types.InlineKeyboardMarkup()
                            btn1 = types.InlineKeyboardButton("Get Number", url="https://t.me/SMSTOSMSBOT?start=start", style=color1)
                            btn2 = types.InlineKeyboardButton("Join Channel", url="https://t.me/+LZrutZRrpbRkNDVl", style=color2)
                            markup.row(btn1, btn2)
                            
                            # বাটন সহ মেসেজ পাঠানো হচ্ছে
                            if service_index % 2 == 0:
                                sent_msg = bot1.send_message(CHAT_ID, message, reply_markup=markup)
                                sender_bot = bot1
                            else:
                                sent_msg = bot2.send_message(CHAT_ID, message, reply_markup=markup)
                                sender_bot = bot2
                            print(f"   ✅ {sid} পাঠানো হয়েছে [bot{'1' if service_index % 2 == 0 else '2'}]")

                            # 🗑️ ৩ মিনিট পর অটো-ডিলিট হওয়ার জন্য শিডিউল করা হচ্ছে
                            schedule_delete(sender_bot, CHAT_ID, sent_msg.message_id)
...
if __name__ == "__main__":
    ...
    # 🗑️ Auto-delete scheduler থ্রেড চালু করা হচ্ছে
    threading.Thread(target=delete_scheduler_worker, daemon=True).start()
